...
func TestNoiseMarkerD028RejectsDumbAgentSubstringInData(t *testing.T) {
	root := t.TempDir()
	// id is in the 000-fixture- namespace, filename clean, purpose
	// clean, no cycle-N anywhere — but title names the generator.
	data := `{"id":"000-fixture-positive-governance-accepted","title":"Positive fixture generated by dumb-agent cycle","owner":"kimjooyoon","status":"accepted","source":"top_down","scope":{"bounded_contexts":["platform"],"areas":["governance"]},"evidence":[{"kind":"file","ref":"x"}],"affected_repos":["agent-cluster-contracts"],"ssot_owner":"agent-cluster-contracts","generated_artifacts":[],"guards":[],"examples":["x"],"counterexamples":[],"created_at":"2026-06-03"}`
	write(t, filepath.Join(root, "fixtures/positive/decision/governance-accepted.json"), data)
	write(t, filepath.Join(root, "fixtures/positive/decision/governance-accepted.meta.json"),
		`{"fixture_type":"decision","expected":"pass","purpose":"validate accepted top-down governance decision"}`)
	res, _ := VerifyFixtures(root)
	if res.OK {
		t.Errorf("expected D028 rejection of dumb-agent substring, got OK")
	}
	hit := false
	for _, c := range res.Checks {
		if strings.Contains(c.Reason, "noise marker (D028)") && strings.Contains(c.Reason, "dumb-agent") {
			hit = true
		}
	}
	if !hit {
		t.Errorf("expected 'noise marker (D028) ... dumb-agent' reason, got %+v", res.Checks)
	}
}

func TestNoiseMarkerD028CaseInsensitive(t *testing.T) {
	cases := []string{
		"DUMB-AGENT",
		"Dumb-Agent",
		"dumb-AGENT",
		"contains dumb-agent here",
	}
	for _, variant := range cases {
		t.Run(variant, func(t *testing.T) {
			root := t.TempDir()
			data := `{"id":"000-fixture-positive-test","title":"` + variant + `","owner":"t","status":"accepted","source":"top_down","scope":{"bounded_contexts":[],"areas":["governance"]},"evidence":[{"kind":"file","ref":"x"}],"affected_repos":["agent-cluster-contracts"],"ssot_owner":"agent-cluster-contracts","generated_artifacts":[],"guards":[],"examples":["x"],"counterexamples":[],"created_at":"2026-06-03"}`
			write(t, filepath.Join(root, "fixtures/positive/decision/x.json"), data)
			write(t, filepath.Join(root, "fixtures/positive/decision/x.meta.json"),
				`{"fixture_type":"decision","expected":"pass","purpose":"case-insensitive marker check coverage"}`)
			res, _ := VerifyFixtures(root)
			if res.OK {
				t.Errorf("variant %q: expected D028 rejection, got OK", variant)
			}
		})
	}
}

func TestNoiseMarkerD028DoesNotMatchInnocentText(t *testing.T) {
	root := t.TempDir()
	// "agent" alone, "dumb agent" with space, "agent-dumb" reversed —
	// none should trip the literal `dumb-agent` substring rule.
	write(t, filepath.Join(root, "fixtures/positive/decision/innocent.json"), validDecisionJSON())
	write(t, filepath.Join(root, "fixtures/positive/decision/innocent.meta.json"),
		`{"fixture_type":"decision","expected":"pass","purpose":"verify decision validator handles a clever agent-driven workflow with dumb errors"}`)
	res, _ := VerifyFixtures(root)
	if !res.OK {
		t.Errorf("innocent text containing 'agent' and 'dumb' separately should pass, got %+v", res.Checks)
	}
}
