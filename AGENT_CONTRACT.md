...
#### Purpose rules (D015 + D017 + D018 + D020 + D023 + D026 + D028)
...
2. **Structural noise marker (D023 + D026 + D028)**: rejected on
   FIRST occurrence (no banlist seeding required). Six sub-checks
   in order:
   - `cycle-N` token (regex `(?i)\bcycle[\s-]\d+\b`) in `meta.purpose`
     raw text (D023)
   - cycle-N in the fixture filename (D026)
   - cycle-N anywhere in the raw data file content — title, examples,
     evidence.ref (D026)
   - cycle-N anywhere in the raw meta file content outside purpose
     (D026)
   - case-insensitive substring `dumb-agent` in the raw data file
     content (D028) — catches title literals like
     `"Positive fixture generated by dumb-agent cycle"` even after
     cycle-N tokens are scrubbed. Fixture content must describe the
     schema rule under test, not which actor produced the file.
   - For fixtures under `fixtures/*/decision/`, the data file's `id`
     must match `^000-fixture-` (D026, supersedes D023's narrower
     999- ban). The 000-fixture- namespace is reserved for canonical
     designer-authored fixtures.
